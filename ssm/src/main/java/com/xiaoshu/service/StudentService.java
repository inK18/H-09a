...
public class StudentService {
	...
	public List<School> findSchool() {
		...
		List<School> slist=studentMapper.findSchool();
		return slist;
	}

	// 通过用户名判断是否存在，（新增时不能重名）
		public Student existUserWithUserName(String studentName) throws Exception {
			List<Student> userList = studentMapper.selectByName(studentName);
			return userList.isEmpty()?null:userList.get(0);
		};
		...
		public List<StudentVo> getEcharts() {
			// TODO Auto-generated method stub
			List<StudentVo> svlist=studentMapper.getEcharts();
			return svlist;
		};
